 ...
 * Plugin URI: https://kerrydavis.com
 * Description: This will add a welcome bar at the top of your website
 * Version: 1.0
 * Author: Kerry Davis
 * Author URI URI: https://kerrydavis.com
 */

  //Add bar after the opening of the body

  add_action('wp_body_open', 'tb_head');

  function get_user_or_website_name() {
    if ( !is_user_logged_in() ) {
      ...
      return 'Welcome back ' . $curren_user -> user_login;
    }
  }

  function tb_head() {
    echo '<h3 class="tb">' . get_user_or_website_name() . ' </h3>';
  }
  ...
  function topbar_page_html() { ?>
    <div class="wrap top-bar-wrapper">
      <form method="post" action="options.php">
        ...
        <input name="topbar_field" id="topbar_field_eat" type="text" value=" 
        <?php echo get_option('topbar_field'); ?>
        ">
        <?php submit_button(); ?>
      </form>
    </div>
  ...
  add_action('admin_head', 'topbar_style');

  function topbar_style() {
    echo '<style>
